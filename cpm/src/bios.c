...
{
  ConsoleParams *cp = z80->console_params; // kb
  A = (cp->console_peek_char (cp->context) >= 0);
...
//	input(z80, 0x00, 0x00, &A);
  ...
  ConsoleParams *cp = z80->console_params; // kb
  A = cp->console_get_char_timeout (cp->context, -1);
  if (A == 10) A = 13; // Not sure about this
...
realizedisk(z80info *z80)
{
	...
	if (z80->drives[drive] == NULL)
	{
		...
		fp = MYFOPEN(drivestr, "rb+");

		if (fp == NULL)
			fp = MYFOPEN(drivestr, "wb+");

		if (fp == NULL)
		{
			MYFPRINTFERR(stderr, "seldisc(): Cannot open file '%s'!\r\n",
					drivestr);
			return;
		}
		...
		{
			MYFPRINTFERR(stderr, "seldisc(): Cannot stat file '%s'!\r\n",
					drivestr);
			MYFCLOSE(fp);
			return;
		}
		...
		if (secs == 0)
		{
			...
			if (MYFWRITE(buf, 1, SECTORSIZE, fp) != SECTORSIZE)
			{
				MYFPRINTFERR(stderr, "seldisc(): Cannot create file '%s'!\r\n",
						drivestr);
...
seldisc(z80info *z80)
{
	...
	if (C >= MAXDISCS)
	{
		MYFPRINTFERR(stderr, "seldisc(): Attempt to open bogus drive %d\r\n",
			C);
		return;
	}
...
settrack(z80info *z80)
{
	...
	if (z80->track < RESERVEDTRACKS || z80->track >= tracks)
		MYFPRINTFERR(stderr, "settrack(): bogus track %d!\r\n",
				z80->track);
}
...
setsector(z80info *z80)
{
	...
	if (z80->sector < SECTOROFFSET || z80->sector > sectors)
		MYFPRINTFERR(stderr, "setsector(): bogus sector %d!\r\n",
				z80->sector);
}
...
rdsector(z80info *z80)
{
	...
	if (fp == NULL)
	{
		MYFPRINTFERR(stderr, "rdsector(): file/drive %d not open!\r\n",
			drive);
		A = 1;
		return;
	}
	...
	if (MYFSEEK(fp, offset, SEEK_SET) != 0)
	{
		MYFPRINTFERR(stderr, "rdsector(): MYFSEEK failure offset=0x%lX!\r\n",
			offset);
		A = 1;
		return;
	}

	n = MYFREAD(&(z80->mem[z80->dma]), 1, SECTORSIZE, fp);

	if (n != SECTORSIZE)
	{
		MYFPRINTFERR(stderr, "rdsector(): read failure %d!\r\n", n);
		A = 1;
	}
	else
		A = 0;
...
wrsector(z80info *z80)
{
	...
	if (fp == NULL)
	{
		MYFPRINTFERR(stderr, "wrsector(): file/drive %d not open!\r\n",
			drive);
		A = 1;
		return;
	}
	...
	if (len && offset > len)
	{
		...
		if (MYFSEEK(fp, len, SEEK_SET) != 0)
		{
			MYFPRINTFERR(stderr, "wrsector(): MYFSEEK failure offset=0x%lX!\r\n",
				len);
			A = 1;
			return;
		}
		...
		while (offset > len)
		{
			if (MYFWRITE(buf, 1, SECTORSIZE, fp) != SECTORSIZE)
			{
				MYFPRINTFERR(stderr, "wrsector(): write failure!\r\n");
				A = 1;
				return;
			}
	...
	if (MYFWRITE(&(z80->mem[z80->dma]), 1, SECTORSIZE, fp) != SECTORSIZE)
	{
		MYFPRINTFERR(stderr, "wrsector(): write failure!\r\n");
		A = 1;
	}
...
secttran(z80info *z80)
{
	if (z80->drive < NUMHDISCS)
	{
		...
		if (BC >= HDSECTORSPERTRACK)
			MYFPRINTFERR(stderr, "secttran(): bogus sector %d!\r\n", BC);
	}
	else
	{
...
MYFILE *cpm_file[CPM_FILES];

int cpm_file_alloc(MYFILE *f)
{
	int x;
...
openunix(z80info *z80)
{
	...
	*fp = 0;
	...
	if ((fd = MYFOPEN(filename, "rb+")) == NULL)
		if ((fd = MYFOPEN(filename, "rb")) == NULL)
			return;
...
createunix(z80info *z80)
{
	...
	*fp = 0;
	...
	if ((fd = MYFOPEN(filename, "wb+")) == NULL)
		return;
...
rdunix(z80info *z80)
{
	...
	if (MYFSEEK(fd, (long)blk << 7, SEEK_SET) != 0)
		return;

	i = MYFREAD (cp, 1, SECTORSIZE, fd);
	size = i;
...
wrunix(z80info *z80)
{
	...
	i = MYFWRITE (cp, 1, size = SECTORSIZE, fd);
...
bios(z80info *z80, unsigned int fn)
{
	...
	{
		MYFPRINTFERR(stderr, "Illegal BIOS call %d\r\n", fn);
		return;
	}
